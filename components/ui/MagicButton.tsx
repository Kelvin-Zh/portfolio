...
const MagicButton = ( {title, icon, position, handleClick, otherCLasses}: {
  ...
  <span className={`inline-flex h-full w-full text-l cursor-pointer gap-2 items-center justify-center rounded-lg bg-slate-950 px-7 text-sm font-medium text-white backdrop-blur-3xl`}>
    {position === "left" && icon}
    {title}
    {position === "right" && icon}
  </span>

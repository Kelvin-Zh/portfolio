...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  const { title, description, subtitle } = slide
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      ...
      >
        ...
        >
          <h2 className="text-lg md:text-2xl lg:text-4xl font-semibold relative mb-4">{title}</h2>
          <p className="text-sm md:text-base lg:text-lg opacity-90 mb-6 max-w-md mx-auto">{description}</p>
          <p className="text-xs md:text-sm italic text-white/70">{subtitle}</p>
        </article>
      </li>
    </div>
  )
